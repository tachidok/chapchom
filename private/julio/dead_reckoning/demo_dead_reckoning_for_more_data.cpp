...
// Data to evaluation
#define READ_AND_OUTPUT_PROCESSED_INFO_FROM_MORE_DATA
...
// ===================================================================
void add_gaussian_noise(const unsigned n_data,
                        double *aligned_time,
                        ...
                        double *aligned_acc_signal_z)
{
 ...
 for (unsigned i = 0; i < n_data; i++)
  {
   aligned_gyro_signal_x[i]+=dist(generator);
   aligned_gyro_signal_y[i]+=dist(generator);
   aligned_gyro_signal_z[i]+=dist(generator);
   aligned_acc_signal_x[i]+=dist(generator);
   aligned_acc_signal_y[i]+=dist(generator);
   aligned_acc_signal_z[i]+=dist(generator);
  }
...
int main(int argc, char *argv[])
{
 ...
 const unsigned Initial_index = 0;
 //const unsigned Final_index = 720; // One minute of data
 const unsigned Final_index = 7202; // Ten minute of data
...
#ifdef APPLY_GAUSSIAN_NOISE
   add_gaussian_noise(n_data,
                      aligned_time,
                      aligned_gyro_signal_x,
                      aligned_gyro_signal_y,
                      aligned_gyro_signal_z,
                      ...
                      aligned_acc_signal_y,
                      aligned_acc_signal_z);
#endif // #ifdef APPLY_GAUSSIAN_NOISE
