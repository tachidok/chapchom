...
int main(int argc, char *argv[])
{
 ...
 const double n_steps = t_final * 20.0; // * readings per second
 // Get the step size
 ...
 y[0][4] = 1.7023341886 * M_PI / 180.0;  // Initial roll
 y[0][5] = 0.9397878122 * M_PI / 180.0;  // Initial pitch
 y[0][6] = 40.823029194 * M_PI / 180.0;  // Initial yaw
 ...
 acc_angles[0] = atan2(acc[1], acc[2]) + M_PI;
 ...
 //y[0][4] = alpha * y[0][4] + (1.0 - alpha) * acc_angles[0];
 //y[0][5] = alpha * y[0][5] + (1.0 - alpha) * acc_angles[1];
 //y[0][6] = alpha * y[0][6] + (1.0 - alpha) * acc_angles[2];
 ...
 fill_rotation_matrices(R, R_t, y[0][4], y[0][5], y[0][6]); // tachidok
